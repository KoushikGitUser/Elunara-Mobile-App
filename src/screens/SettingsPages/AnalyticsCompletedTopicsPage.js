...
const AnalyticsCompletedTopicsPage = ({
  ...
  const [sortBy, setSortBy] = useState("last_accessed_at");
        ...
        settingsStates.analyticsTopics.length > 0 ? (
            ...
            <TouchableOpacity key={topicIndex} style={styles.container}>
              <View
                style={[
                  styles.card,
                  {
                    paddingVertical: topic.last_accessed_at ? 10 : 20,
                    backgroundColor: topic.last_accessed_at
                      ? "#F3F3F3"
                      : "white",
                    borderColor: "#D3DAE5",
                    borderWidth: topic.last_accessed_at ? 0 : 1,
                  },
                ]}
              >
                ...
                {topic.last_accessed_at ? (
                  <View style={styles.rightSection}>
                    <Text style={styles.date}>
                      {new Date(topic.last_accessed_at).toLocaleDateString()}
                    </Text>

                    <Text style={styles.time}>
                      {new Date(topic.last_accessed_at).toLocaleTimeString()}
                    </Text>
                  </View>
                ) : (
          ...
          >
            <View style={styles.noResultMain}>
              ...
              >
                You haven't explored any topics in this subject so far. Start
                diving in now to build your knowledge!
              </Text>
            </View>
          </View>
        )}
