...
const BulkRemoveFromRoomConfirmPopup = ({ selectedChatIds = [], onComplete }) => {
  ...
  const handleBulkRemoveFromRoom = async () => {
    ...
    try {
      // Remove each chat from room sequentially
      ...
      setIsLoading(false);
      dispatch(setToggleBulkRemoveFromRoomConfirmPopup(false));

      // Show success toast
      triggerToast(
        "Chats Removed",
        `${chatCount} chat${chatCount > 1 ? "s" : ""} removed from room`,
        "success",
        3000
      );

      // Refetch chats in the room
      if (roomUuid) {
        dispatch(
          commonFunctionForAPICalls({
            method: "GET",
      ...
      // Call onComplete callback to reset selection state
      if (onComplete) {
        onComplete();
      }
    } catch (error) {
  ...
  return (
    <Modal
      visible={toggleStates.toggleBulkRemoveFromRoomConfirmPopup}
      transparent={true}
      animationType="slide"
      onRequestClose={() => dispatch(setToggleBulkRemoveFromRoomConfirmPopup(false))}
    >
      <Toaster />
      <View style={styles.container}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={1}
          onPress={() => dispatch(setToggleBulkRemoveFromRoomConfirmPopup(false))}
        />
        ...
        <View style={styles.modalSheet}>
          ...
          <View style={styles.content}>
            ...
            <Text style={[styles.title, { fontFamily: "Mukta-Bold" }]}>
              Remove {chatCount} chat{chatCount > 1 ? "s" : ""} from Room?
            </Text>

            {/* Description */}
            <Text style={[styles.description, { fontFamily: "Mukta-Regular" }]}>
              These chats will be removed from this room but will still be available in your chats.
            </Text>
...
export default BulkRemoveFromRoomConfirmPopup;
